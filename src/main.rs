...
fn attach_note_from_log(
    ...
    // Get user email from onboarding config
...
fn run_reset() -> Result<()> {
    output::action("Resetting", "configuration/state");
    ...
    for key in global_keys {
        if let Err(e) = git::config_unset_global(key) {
            output::note(&format!("Warning: failed to unset global {}: {}", key, e));
        }
    }
    ...
    if git::repo_root().is_ok() {
        for key in local_keys {
            if let Err(e) = git::config_unset(key) {
                output::note(&format!("Warning: failed to unset local {}: {}", key, e));
            }
        }
    }
    ...
    output::note("To remove binary: cargo uninstall cadence");
    Ok(())
}
...
fn run_status_inner(w: &mut dyn std::io::Write) -> Result<()> {
    ...
    match onboarding::get_email() {
        Some(email) => {
            output::detail_to_with_tty(w, &format!("User email: {}", email), false);
        }
        None => {
            output::detail_to_with_tty(
                w,
                "User email: (not set; run `cadence onboard` or set git user.email)",
                false,
            );
        }
    }
    ...
    output::detail_to_with_tty(w, &format!("Scope: {}", scope_mode.as_str()), false);
    if matches!(scope_mode, onboarding::ScopeMode::Selected) {
        let selected = onboarding::get_selected_repos();
        if selected.is_empty() {
            output::detail_to_with_tty(w, "Scope selected repos: (none)", false);
        } else {
            output::detail_to_with_tty(
                w,
                &format!("Scope selected repos: {}", selected.len()),
                false,
            );
        }
    }
    ...
    if repo_root.is_some() {
        match git::config_get("ai.session-commit-linker.autopush") {
            ...
            _ => {
                output::detail_to_with_tty(w, "Auto-push: (not set)", false);
            }
        }
    } else {
...
mod tests {
    ...
    fn cli_parses_onboard() {
        let cli = Cli::parse_from(["cadence", "onboard"]);
    ...
    fn cli_parses_scope_set() {
        let cli = Cli::parse_from(["cadence", "scope", "set", "selected"]);
        assert!(matches!(
            cli.command,
            Command::Scope {
    ...
    fn cli_parses_scope_add() {
        let cli = Cli::parse_from(["cadence", "scope", "add", "."]);
        assert!(matches!(
            cli.command,
            Command::Scope {
